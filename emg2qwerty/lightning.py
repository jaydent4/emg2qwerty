...
class GRUEncoder(nn.Module):
    """A simple GRU-based encoder.

    Accepts inputs of shape (T, N, input_size) and returns outputs of shape
...
class GRUCTCModule(pl.LightningModule):
    """A CTC model with optional GRU encoding.

    This model uses the same front-end feature extractor as `TDSConvCTCModule`.
    Instead of a TDS convolutional encoder, it uses a stacked GRU to encode
    temporal features.
    """
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            # (T, N, encoder_output_size)
            nn.Linear(encoder_output_size, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
    ...
    def _step(
        ...
        # The GRU encoder does not change temporal length, so emission_lengths
        # are the same as input_lengths.
...
class ConvRNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            # (T, N, num_classes)
            nn.Linear(rnn_hidden_size * 2, charset().num_classes),  # *2 because bidirectional
            nn.LogSoftmax(dim=-1),
        )
    ...
    def _step(
        ...
        # Calculate difference in lengths (should be 0 because we pad the Conv1d)
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
...
class CNNTransformerCTCModule(pl.LightningModule):
    ...
    NUM_BANDS: ClassVar[int] = 2
    FREQ_BINS: ClassVar[int] = 33  # n_fft // 2 + 1 = 64 // 2 + 1
